...
const SignUp = () => {
	...
	const onFinish = async () => {
		...
		if(loginPw.val() != loginPwConf.val()){
			$("#login-pw-conf-need").text("비밀번호와 비밀번호 확인이 일치하지 않습니다.");
			emptyInput('#login-pw-conf');
			return;
		}
	...
	return (
		<div style={{backgroundColor:'#F6F6F6', paddingTop:70, height:"100vh"}}>
			<Header logout={false}/>
			<div style={{width:335, marginLeft:'auto', marginRight:'auto'}}>
				<div style={{marginTop:50, marginBottom:20, fontWeight:'bold', fontSize:"20px", lineHeight:"31px", letterSpacing:-1, color:'#042045', backgroundColor:'#F6F6F6'}}>
					회원가입
				</div>
				...
				<div style={{marginTop:20}}>
					<input id="login-pw-conf" type="password" style={{width:335}} placeholder="비밀번호를 입력하세요" onChange={() =>fillInput('#login-pw-conf')}/>
					<span id='login-pw-conf-need' style={{float:'left', fontSize:'12px', color:'#FF2E4C', display:'none'}}>
						비밀번호 확인을 입력해주세요.
					</span>
				</div>
				...
				<div style={{marginTop:20}}>
					...
					&nbsp;/&nbsp;  
					<a onClick={()=> setFindModal(true)} style={{textDecoration:'none', fontSize:"13px", lineHeight:"15px", color:'#3E3F41', cursor:'pointer'}} >아이디 & 비밀번호 찾기</a>
				</div>
			</div>
			<Modal
				style={{
			...
			>
				<div style={{textAlign:'center', padding:20}}>
					<div style={{ fontSize: "20px", color: "#ff2e4c", fontWeight: "bold", marginBottom:10 }}>
						- 	NewOpen 매니저 -
					</div>
					...
					<div style={{ fontSize: "20px" }}>
						해당 이메일 & 번호로 아이디를 문의 주시면 비밀번호를 최대한 빠른시간내에 찾아드리겠습니다.
					</div>
				</div>
			</Modal>
